...
class Product extends React.Component {
  ...
  async clickBuy(title, price) {
    try {
      ...
      window.location.href =
        "https://www.mercadopago.com.ar/checkout/v1/redirect?pref_id=22115028-e627c589-3404-472a-9171-e315d7e0476f";
    } catch (err) {
      console.log(err);
    }
  }
  ...
  render() {
    ...
    return (
      <>
        <Navbar
          handleCallback={(call) => this.handleCallback(call)}
          Buttonback={() => this.handleButton()}
        />
              ...
              <div className="itemInf">
                <div>
                  <p className="condition">
                    {" "}
                    {condition} | {sold} vendidos{" "}
                  </p>
                  <p className="itemTitle">{title}</p>
                  {poc.map((pic, key) => {
                    return (
                      <div key={key} className="imgRes">
                ...
                <div>
                  ...
                  <p className="itemCuote">
                    Paga hasta en 12 cuotas de ${Math.round(price / 12)}
                  </p>
                  {price <= 3499 ? (
                    <p> Costo de envio </p>
                  ) : (
                    <p className="itemSend">Envio gratis</p>
                  )}
                  <p className="itemCuote">stock disponible</p>
                  <p className="itemCuote">
                    Cantidad:({available_quantity} disponibles)
                  </p>
                </div>
                <div>
                  <button
                    className="buy Button"
                    onClick={() => this.clickBuy(title, price)}
                  >
                    Comprar ahora
                  </button>
                  <button
                    onClick={() => this.Click(title, price, poc)}
